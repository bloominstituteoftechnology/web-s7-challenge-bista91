...
export default function Form() {
  ...
  const onChange = (event) => {
    ...
    //for checkboxes we will update the based on whether its checked or not.
    ...
    //validate the update value and set the error message
  ...
  const onCheckboxChange = (event) => {
    ...
    let newTopings = [...formValue.toppings];
    if(checked) {
      newTopings = [...newTopings, value];
    }
    else {
      newTopings = newTopings.filter((toping) => toping !== value);
    };
    setFormValue({ ...formValue, toppings: newTopings});
  };
      ...
      {failure && (
        <div>
          <p>
            ...
            {formValue.size} pizza with {formValue.toppings.length} toppings is on the way.
          </p>
          {/* <p>Error Date: {JSON.stringify(failure.data)}</p> */}
        </div>
      )}
